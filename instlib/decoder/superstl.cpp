...
namespace superstl {
  ...
  bool odstream::open(const char* filename, bool append, int bufsize) {
    if (fd >= 0) close();
    fd = open(filename, O_RDWR | O_CREAT | ((append) ? O_APPEND : O_TRUNC), 0644);
  ...
  int idstream::fillbuf() {
    ...
    if (head < tail) {
      ...
      int hin = ::read(fd, &buf[tail], hibytes);
      if (hin < 0) hin = 0;
  ...
  int idstream::read(void* dest, int count) {
    if (!buf) return ::read(fd, dest, count);
