...
#define PRETEXT_ALL_FLAGS_IN "pushq %[rcflags]; popfq; "
...
inline void ctzclzop(IssueState& state, W64 ra, W64 rb, W64 rc, W16 raflags, W16 rbflags, W16 rcflags) {
  ...
  T rt = f(ra, rb, rc, raflags, rbflags, rcflags, cf, of);
  state.reg.rddata = x86_merge<T>(ra, rt);
  // cfallin fix -- on no bit found, do a full width passthrough (don't change dest),
  // to match Intel and Bochs behavior. Don't go through x86_merge<>, which will
  // truncate high bits for 32-bit size in 64-bit mode.
  if (!rb) state.reg.rddata = ra; 
  state.reg.rdflags = (((T)rb) == 0) ? FLAG_ZF : 0;
  capture_uop_context(state, ra, rb, rc, raflags, rbflags, rcflags, ptlopcode, log2(sizeof(T)));
}

make_exp_aluop(exp_op_ctz, (rd = (rb) ? lsbindex64(rb) : (ra)));
make_anyop_all_sizes(OP_ctz, implmap_ctz, ctzclzop, exp_op_ctz, ZAPS);

make_exp_aluop(exp_op_clz, (rd = (rb) ? msbindex64(rb) : (ra)));
...
#ifdef __x86_64__
#ifdef DARWIN
// on Mac OS X toolchain, we need 'movd' rather than 'movq'. It assembles to the same thing,
// but the assembler doesn't recognize 'movq' for XMM<->GPR moves.
#define MOV_TO_XMM "movd" 
#else
#define MOV_TO_XMM "movq"
#endif
#define W64_CONSTRAINT "r"
#else
// 32-bit x86
#define MOV_TO_XMM "movd"
#define MOV_TO_XMM_MEM "movd"
