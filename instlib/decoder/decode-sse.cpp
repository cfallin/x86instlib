...
#undef Q
    ...
    if unlikely (uop == OP_permb) {
      ...
      int x = 1;
      if unlikely(use_mmx) { if (hi) x = 0; hi = false; }
      //
      ...
      //
      this << TransOp(OP_permb, REG_temp2, rdreg+hi, rareg+hi, REG_imm, 3, 0, permute_control_info[sel][x]);
      if likely (!use_mmx)
      ...
      if likely (!use_mmx) {
          ...
          this << TransOp(uop, rdreg, rdreg, rareg, REG_zero, sizeshift);
      }
    }
    break;
  }
