...
bool TraceDecoder::decode_fast() {
  ...
  case 0x190 ... 0x19f: {
    ...
    if (rd.type == OPTYPE_REG) {
      r = temp_r = arch_pseudo_reg_to_arch_reg[rd.reg.reg];
    ...
    TransOp transop(OP_set, temp_r, cctfr.ra, cctfr.rb, (rd.type == OPTYPE_MEM) ? REG_zero : r, 0);
    transop.cond = condcode;
    this << transop, endl;

    if (rd.type == OPTYPE_REG && reginfo[rd.reg.reg].hibyte) {
        this << TransOp(OP_maskb, r, r, temp_r, REG_imm, 3, 0, MaskControlInfo(56, 8, 56));
    }
