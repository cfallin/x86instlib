...
#define __isgreater(x, y)	__builtin_isgreater(x, y)
#define __isgreaterequal(x, y)	__builtin_isgreaterequal(x, y)
#define __isless(x, y)		__builtin_isless(x, y)
#define __islessequal(x, y)	__builtin_islessequal(x, y)
#define __islessgreater(x, y)	__builtin_islessgreater(x, y)
#define __isunordered(u, v)	__builtin_isunordered(u, v)
...
#undef fpclassifyf
  inline int fpclassifyf(float x) {
    ...
    if (u.w == 0)
      ...
      retval = u.w > 0x7f800000 ? __FP_NAN : __FP_INFINITE;
    
    return retval;
  }

#undef isnormal
  inline bool isnormal(double x) { return (fpclassify(x) == __FP_NORMAL); }
};
#endif
